...
import com.bupt.common.flow.constant.FlowTaskType;
...
public class FlowEntryController {
    ...
    private static final Logger LOGGER = Logger.getLogger(FlowEntryController.class.getName());
    ...
    public ResponseResult<Void> update(@MyRequestBody FlowEntryDto flowEntryDto) {
        ...
        List<FlowVaribleDisplayDto> flowVaribleDisplayDtosList = flowEntryDto.getVariableDisplay();
        List<FlowVariableDisplay> flowVariableDisplayList = new ArrayList<FlowVariableDisplay>();

        for(int i = 0; i < flowVaribleDisplayDtosList.size(); i++){
            flowVariableDisplayList.add(MyModelUtil.copyTo(flowVaribleDisplayDtosList.get(i), FlowVariableDisplay.class));
        }
        if (flowEntryDto.getIfVariableDisplay() && !flowVariableDisplayService.updateByEntry(flowEntry.getEntryId().toString(), flowVariableDisplayList)) {
    ...
    public ResponseResult<List<Map<String, Object>>> listDict(FlowEntry filter) {
        List<FlowEntry> resultList = flowEntryService.getFlowEntryList(filter, null);
        return ResponseResult.success(
